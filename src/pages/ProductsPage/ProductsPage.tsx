...
const DropdownFilter: React.FC<DropdownFilterProps> = ({
  ...
  return (
    <div className="dropdown-filter">
      ...
      {isOpen && (
        <div
          className={`dropdown-content ${multiColumn ? "four-columns" : ""}`}
        >
          {items.map((item) => {
...
const ProductsPage: React.FC = () => {
  ...
  const [selectedCategory, setSelectedCategory] =
    React.useState<string>(categoryFromUrl);
  ...
  const [selectedConnectionTypes, setSelectedConnectionTypes] = React.useState<
    string[]
  >([]);
  const [selectedCompatibility, setSelectedCompatibility] = React.useState<
    string[]
  >([]);
  const [selectedFormFactors, setSelectedFormFactors] = React.useState<
    string[]
  >([]);
  const [selectedLedTypes, setSelectedLedTypes] = React.useState<string[]>([]);
  const [selectedFeatures, setSelectedFeatures] = React.useState<string[]>([]);
  // Headset specific filters
  const [selectedHeadsetTypes, setSelectedHeadsetTypes] = React.useState<
    string[]
  >([]);
  ...
  const [selectedScreenSizes, setSelectedScreenSizes] = React.useState<
    string[]
  >([]);
  const [selectedRefreshRates, setSelectedRefreshRates] = React.useState<
    string[]
  >([]);
  const [selectedResolutions, setSelectedResolutions] = React.useState<
    string[]
  >([]);
  const [selectedResponseTimes, setSelectedResponseTimes] = React.useState<
    string[]
  >([]);
  const [selectedPanelTypes, setSelectedPanelTypes] = React.useState<string[]>(
    [],
  );
  const [selectedMonitorFeatures, setSelectedMonitorFeatures] = React.useState<
    string[]
  >([]);
  // USB filters state
  const [selectedStorageCapacities, setSelectedStorageCapacities] =
    React.useState<string[]>([]);
  ...
  const [selectedWriteSpeeds, setSelectedWriteSpeeds] = React.useState<
    string[]
  >([]);
  const [selectedMemoryCardTypes, setSelectedMemoryCardTypes] = React.useState<
    string[]
  >([]);
  ...
  const [selectedContentTypes, setSelectedContentTypes] = React.useState<
    string[]
  >([]);
  const [selectedFormatTypes, setSelectedFormatTypes] = React.useState<
    string[]
  >([]);
  const [selectedLicenseTypes, setSelectedLicenseTypes] = React.useState<
    string[]
  >([]);
  const [selectedSoftwareCompatibility, setSelectedSoftwareCompatibility] =
    React.useState<string[]>([]);
  ...
  const availableMonitorFeatures = React.useMemo(() => {
    ...
    categoryProducts.forEach((product) => {
      if (product.monitor_features) {
        product.monitor_features.forEach((feature) =>
          monitorFeatures.add(feature),
        );
      }
    });
  ...
  const availableSoftwareCompatibility = React.useMemo(() => {
    ...
    categoryProducts.forEach((product) => {
      if (product.software_compatibility) {
        product.software_compatibility.forEach((software) =>
          softwareCompat.add(software),
        );
      }
    });
  ...
  const filteredProducts = React.useMemo(() => {
    ...
    if (selectedConnectionTypes.length > 0) {
      filtered = filtered.filter(
        (product) =>
          ...
          selectedConnectionTypes.some((type) =>
            product.connection_types?.includes(type),
          ),
      );
    }
    ...
    if (selectedCompatibility.length > 0) {
      filtered = filtered.filter(
        (product) =>
          ...
          selectedCompatibility.some((comp) =>
    ...
    if (selectedFormFactors.length > 0) {
      filtered = filtered.filter(
        (product) =>
          product.form_factor &&
          selectedFormFactors.includes(product.form_factor),
      );
    }
      ...
      filtered = filtered.filter(
        (product) =>
          ...
          selectedFeatures.some((feature) =>
            product.features_detailed?.includes(feature),
          ),
      );
    }
    ...
    if (selectedHeadsetTypes.length > 0) {
      filtered = filtered.filter(
        (product) =>
          product.headset_type &&
          selectedHeadsetTypes.includes(product.headset_type),
      );
    }
      ...
      filtered = filtered.filter(
        (product) =>
          ...
          selectedUseCases.some((useCase) =>
            product.use_cases?.includes(useCase),
          ),
      );
    }
    ...
    if (selectedRefreshRates.length > 0) {
      filtered = filtered.filter(
        (product) =>
          ...
          selectedRefreshRates.includes(product.refresh_rate),
      );
    }
    ...
    if (selectedResponseTimes.length > 0) {
      filtered = filtered.filter(
        (product) =>
          ...
          selectedResponseTimes.includes(product.response_time),
      );
    }
    ...
    if (selectedMonitorFeatures.length > 0) {
      filtered = filtered.filter(
        (product) =>
          ...
          selectedMonitorFeatures.some((feature) =>
            product.monitor_features?.includes(feature),
          ),
      );
    }
    ...
    if (selectedStorageCapacities.length > 0) {
      filtered = filtered.filter(
        (product) =>
          ...
          selectedStorageCapacities.includes(product.storage_capacity),
      );
    }
    ...
    if (selectedWriteSpeeds.length > 0) {
      filtered = filtered.filter(
        (product) =>
          product.write_speed &&
          selectedWriteSpeeds.includes(product.write_speed),
      );
    }
    ...
    if (selectedMemoryCardTypes.length > 0) {
      filtered = filtered.filter(
        (product) =>
          product.memory_card_type &&
          selectedMemoryCardTypes.includes(product.memory_card_type),
      );
    }
    ...
    if (selectedContentTypes.length > 0) {
      filtered = filtered.filter(
        (product) =>
          ...
          selectedContentTypes.includes(product.content_type),
      );
    }
    ...
    if (selectedFormatTypes.length > 0) {
      filtered = filtered.filter(
        (product) =>
          product.format_type &&
          selectedFormatTypes.includes(product.format_type),
      );
    }
    ...
    if (selectedLicenseTypes.length > 0) {
      filtered = filtered.filter(
        (product) =>
          ...
          selectedLicenseTypes.includes(product.license_type),
      );
    }
    ...
    if (selectedSoftwareCompatibility.length > 0) {
      filtered = filtered.filter(
        (product) =>
          ...
          selectedSoftwareCompatibility.some((software) =>
            product.software_compatibility?.includes(software),
          ),
      );
    }
    ...
    if (selectedPriceRanges.length > 0) {
      filtered = filtered.filter(
        (product) =>
          product.price_range &&
          selectedPriceRanges.includes(product.price_range),
      );
    }
    ...
    selectedPanelTypes,
    selectedMonitorFeatures,
    selectedStorageCapacities,
    selectedUsbTypes,
    selectedReadSpeeds,
    selectedWriteSpeeds,
    selectedMemoryCardTypes,
    selectedContentTypes,
    selectedFormatTypes,
    selectedLicenseTypes,
    selectedSoftwareCompatibility,
    selectedPriceRanges,
            ...
            <div className="dropdown-filters">
              ...
              {availableSoftwareCompatibility.length > 0 && (
                <DropdownFilter
                  ...
                  onToggle={() =>
                    toggleDropdown("digital-software-compatibility")
                  }
                />
              )}
                ...
                <p className="no-results-description">
                  {searchParams.get("search") ? (
                    <>
                      ...
                      <strong>"{searchParams.get("search")}"</strong>.<br />
                      Vui lòng thử lại với từ khóa khác hoặc điều chỉnh bộ lọc.
                    </>
                  ) : (
                    <>
                      Không có sản phẩm nào phù hợp với các tiêu chí lọc đã
                      chọn.
                      <br />
